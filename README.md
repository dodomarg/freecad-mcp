...
The selection buffer lets the user decide explicitly which objects the AI model
operates on. Objects are handed over from the FreeCAD GUI and stay available
until they are cleared.

* `get_selection_buffer`: Get the current selections from the buffer (non-destructive).
* `get_buffer_status`: Check whether the buffer has selections, plus count and timestamp.
* `clear_selection_buffer`: Clear the buffer after a successful operation.
* `get_selection_workflow_strategy`: Get the recommended workflow for using selections.
* `get_coordinate_handling_strategy`: Get best practices for using coordinates taken from selections.

#### Selection Workflow

1. User selects objects (or faces/edges/vertices) in the FreeCAD GUI
2. User clicks the "Send Selection to MCP" button in the addon toolbar
3. AI model uses `get_selection_buffer()` to retrieve the selection
4. AI model performs operations, retrying with the same selection data if needed
5. After the user confirms satisfaction, the AI model calls `clear_selection_buffer()`
...
- **Order preservation**: Maintains the user's selection sequence
- **No data loss**: Selections persist even if the model isn't actively listening
